...
def __create_tag_api():
    with lcd(env.LOCAL_API_FOLDER):
        ...
        if env.increment == 'no':
            if not console.confirm('WILL BE USED LAST TAG {}?'.format(next_api_tag)):
                utils.abort('FINISHING WITHOUT DEPLOY')
        else:
            if not console.confirm('CONFIRM NEXT API TAG {}?'.format(next_api_tag)):
                utils.abort('FINISHING WITHOUT DEPLOY')
...
def __create_tag_app():
    with lcd(env.LOCAL_APP_FOLDER):
        ...
        if env.increment == 'no':
            if not console.confirm('WILL BE USED LAST TAG {}?'.format(next_app_tag)):
                utils.abort('FINISHING WITHOUT DEPLOY')
        else:
            if not console.confirm('CONFIRM NEXT API TAG {}?'.format(next_app_tag)):
                utils.abort('FINISHING WITHOUT DEPLOY')
...
def prod(increment='rev', project='both'):
    ...
    if project in ['api', 'both']:
        ...
        with cd(env.REMOTE_API_FOLDER):
            ...
            sudo('systemctl restart {}-celery.service'.format(env.PROJECT_NAME))
...
def stag(project='both', branch='staging'):
    __build_env('stag')
    ...
    if project in ['app', 'both']:
        with cd(env.REMOTE_APP_FOLDER):
            ...
            run('git pull origin {}'.format(branch))
            run('bower install')
            run('grunt staging-noug'.format(branch))
